...
var chalk = require( "chalk" ),
    ...
    sURL,
    sSelector,
    fGetImageCallback = function( oError, sSavedImagePath ) {
        if( oError ) {
            console.log( chalk.bold.red( "[ERROR] An error occuring:", oError.message ) );
            process.exit( 1 );
        }
        console.log( chalk.bold.cyan( "• Image saved to:" ), sSavedImagePath.replace( process.cwd(), "." ) );
    };
...
jsdom.env( sURL, function( oError, oWindow ) {
    ...
    console.log( chalk.underline.white( "Results:" ), chalk.bold.green( $targets.length ), "element" + ( $targets.length > 1 ? "s" : "" ) + " found for", chalk.bold.cyan( sSelector ), "selector.", "\n" );
    for( ; $element = $targets[ ++i ] ; ) {
        if( $element.nodeName === "IMG" ) {
            getImage( $element.getAttribute( "src" ), fGetImageCallback );
        } else {
